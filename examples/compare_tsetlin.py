...
def compare():
    ...
    import examples.main_tsetlin as main_tsetlin
    ...
    for run_number in range(compare_number_runs):
        ...
        # Now running for the Tsetlin Machine

        metrics = main_tsetlin.run()
        (
            tsetlin_epoch_train_losses,
            tsetlin_epoch_train_accuracies,
            tsetlin_epoch_test_losses,
            tsetlin_epoch_test_accuracies,
        ) = metrics.run_data()

        train_losses_dataframe = write_run_to_dataframe(
            train_losses_dataframe, tsetlin_epoch_train_losses, "TsetlinMachine", run_number
        )
        train_accuracies_dataframe = write_run_to_dataframe(
            train_accuracies_dataframe,
            tsetlin_epoch_train_accuracies,
        ...
        test_losses_dataframe = write_run_to_dataframe(
            test_losses_dataframe, tsetlin_epoch_test_losses, "TsetlinMachine", run_number
        )
        test_accuracies_dataframe = write_run_to_dataframe(
            test_accuracies_dataframe,
            tsetlin_epoch_test_accuracies,
